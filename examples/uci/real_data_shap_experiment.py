...
#         # Convert SHAP values to DataFrame
#         shap_df = shap_values_to_df(shap_values.values)
...
#     # Save predictions to CSV
#     predictions_df = pd.DataFrame(predictions, columns=["Predictions"])
#     predictions_df.to_csv(f"{output_dir}/predictions_RandomForest_{name}.csv", index=False)

#     print(f"Predictions saved to {output_dir}/predictions_RandomForest_{name}.csv")
